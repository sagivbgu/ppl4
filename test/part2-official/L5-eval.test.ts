...
import { isClosure, makeCompoundSExp,makeTuple } from '../../part2/L5-value';
...
describe('L5 Eval', () => {
    ...
    describe('Primitive Procedures', () => {
        ...
        it('evaluates "values"', () => {
            expect(evalParse("(values 1 2 #t)")).to.deep.equal(makeOk(makeTuple([1,2, true])));
 ...
 //added 
    it('evaluates "let-values" expressions', () => {
        ...
        expect(bind(parseL5("(L5 (let-values  ((( x1 y1) (values 1 2)) (( x2 y2) (values 3 4)))(+ x1 y1 x2 y2)) )"), evalProgram)).to.deep.equal(makeOk(10));
        expect(bind(parseL5("(L5 (define x (values 1 2 3)) (let-values ((( x1 x2 x3) x))(+ x1 x2 x3)) )"), evalProgram)).to.deep.equal(makeOk(6));
...
// //added  example 3 with value
    it('evaluates "define" expressions', () => {
        ...
        expect(bind(parseL5("(L5 (define x (values 1 2 3)) x)"), evalProgram)).to.deep.equal(makeOk(makeTuple([1,2,3])));
