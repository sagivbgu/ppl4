...
import { makeBoolTExp, makeNumTExp, makeProcTExp, makeTVar, makeVoidTExp, parseTE, unparseTExp, makeNonEmptyTupleTExp, makeEmptyTupleTExp } from '../../part2/TExp';
...
// added
        it('unparses Tuple', () => {
            ...
            expect(unparseTExp(makeNonEmptyTupleTExp([makeBoolTExp(), makeProcTExp([makeTVar("T"), makeTVar("T")], makeBoolTExp())]))).to.deep.equal(makeOk("(boolean * (T * T -> boolean))"));
        });
    ...
    it('returns the type of "values" expressions', () => {
        ...
        expect(L5typeof("(values 1 #t(lambda ((x : number)) : number x))")).to.deep.equal(makeOk("(number * boolean * (number -> number))"));
    });

    it('returns the type of "let-values" expressions', () => {
        expect(L5typeof("(let-values ( ( ((x : number) (y : boolean)) (values 1 #t))) y)")).to.deep.equal(makeOk("boolean"));
        expect(L5typeof("(let-values ( ( ((x : number) (y : boolean)) (values 1 #t)) (((d : number) (e : number)) (values 3 4))) e)")).to.deep.equal(makeOk("number"));
        expect(L5typeof("(let-values  ( (((x : number) (y : number)) (values 1 2)) ) (lambda((a : number)) : number (+ (* x a) y)))")).to.deep.equal(makeOk("(number -> number)"));
    });
